...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    PIP_RETRIES=5 \
    LZAGENT_HOST=0.0.0.0 \
    LZAGENT_PORT=8020 \
    LZAGENT_DATA_DIR=/app/data \
    LZAGENT_CONFIG_DIR=/app/config \
    LZAGENT_WORKSPACE_DIR=/app/workspace \
    LZAGENT_PACKAGES_DIR=/app/.packages \
...
RUN chown -R zlagent:zlagent /app/backend
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
    CMD python -c "import os, urllib.request; urllib.request.urlopen(f'http://127.0.0.1:{os.environ.get(\"LZAGENT_PORT\",\"8020\")}/api/health', timeout=5).read()" || exit 1
