...
description: >
  ...
  transport, stay, budget, pitfalls, and packing. Returns a single rich-
  content card, which (a) avoids WeChat rate limits (would otherwise fan
  out 6+ messages),
...
metadata:
  ...
  zlagent:
    ...
    wiki_cache:
      ...
      ttl_days: 30
    # opt into RichMessage output. The agent loop appends the
    ...
    # a half-rendered JSON fence mid-stream. Multi-section streaming
    # has been retired for travel-guide because it fanned out 6+
    # messages and tripped WeChat rate limits.
    rich_output: true
    # atomic-fact crystallization. After each successful
...
You are LZAgent in **travel-guide mode**. The user has asked for travel
