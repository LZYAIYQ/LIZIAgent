...
1. **Source priority** (cheapest + highest signal first):
   - **scholar_search** — Google Scholar search via `scholar_search` tool.
     Best overall coverage across venues and years. Returns title,
     authors, year, citation count, abstract, and URL. Use this as the
   ...
   - **Semantic Scholar Graph API** — `https://api.semanticscholar.org/graph/v1/paper/search?query=<topic>&limit=20&fields=title,authors,year,venue,citationCount,abstract,url`
     ...
     on without it.
   - **web_search** — only as a last fallback when scholar + arxiv + S2
     all yielded fewer than `count` candidates. Query template:
...
4. **Tool budget**: aim for ≤ 3 tool calls total. List-task
   ...
   for retries, not for browsing every candidate. Prefer
   `scholar_search` first (one call, rich metadata), then fall back
   to arxiv / S2 only if scholar results are thin.
...
- Google Scholar may rate-limit or block requests. If `scholar_search`
  returns errors, degrade to arxiv + S2 silently. Configure
  `LZAGENT_SCHOLAR_PROXY` for networks that block Google Scholar.
- Semantic Scholar 429 is common; honour the `Retry-After` header
  and degrade silently to arxiv-only.
- DDG / Bing captcha will trigger the provider chain in
  `web_search`; if all 3 web providers fail, fall back to whatever
  scholar + arxiv + S2 produced even if `< count`.
