...
description: |
  ...
  Hermes' mcporter skill — same flow, LZAgent-tool-aware.
version: 0.1.0
tags:
  - mcp
  - discovery
...
3. Call `code_execution` with `language='shell'` to query the live MCP
   ...
   # registries and to MCP servers directly. LZAgent's Docker image
...
4. Use the most useful subcommand for the task:
   ...
   # LZAgent's own /api/mcp/servers is the real source of truth — use
   ...
   # verify capability fit without polluting LZAgent's registry.
...
5. Pick at most **three** candidates. Prefer:
   ...
   - Require interactive OAuth in a browser (LZAgent has no UI for
...
8. **First**, install the package binary into LZAgent's managed prefix
   ...
   LZAgent will pop a confirm prompt; the user replies `yes`. The
...
9. **Second**, attach the now-installed binary. Translate the install
   ...
   - `env={...}` — only the env vars the user explicitly authorised.
     **Do not** fill in NODE_OPTIONS, PYTHONSTARTUP, or anything in
     LZAgent's env-var blocklist; the validator will refuse those.
   ...
   Call `mcp_manage(action='add', ...)`. LZAgent will pop another
...
- No env keys from the blocklist were passed.
...
- LZAgent's `mcp_manage` tool — see `backend/tools/builtins/mcp_manage.py`
- LZAgent's lifecycle service — see `backend/mcp/lifecycle.py`
