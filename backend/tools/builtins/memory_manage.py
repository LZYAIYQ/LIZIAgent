...
# performative-write patterns. These are the canonical shapes
...
class MemoryManageTool(Tool):
    ...
    def __init__(
        ...
        *,
        ...
        # hard cap on a single fact entry. Hermes' built-in
    ...
    def _remember(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        # Hermes-style brevity guard. The user explicitly asked
        ...
        # performative-write guard. Reject contents that look
    ...
    def _infer_knowledge_base_id(
        ...
        *,
        ...
        # Multi-user: use user's knowledge_base_id from turn context
        from ...agent.tool_context import current_turn_context
        ctx = current_turn_context()
        if ctx and ctx.platform and ctx.user_id:
            try:
                from ...db.users import UserManager
                user_kb = f"user_{ctx.platform}_{ctx.user_id}"[:64]
                return user_kb
            except Exception:
                pass
