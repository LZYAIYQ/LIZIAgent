...
function-calling loop:
    ...
    2. If the model emits ``tool_calls``:
       ...
       b. The first ``confirm`` tool encountered (in *interactive* turns)
          ...
          ``backend.agent.decisions.classify_decision`` resumes the loop
...
from .correction import CorrectionSignal
from .failure_learning import FailureLearner
from ..skills.loader import SkillLoader
from ..tools import ToolRegistry
from .tool_context import TurnContext, set_turn_context
...
# The canonical home is ``loop_prompts``; ``loop`` keeps these visible so
# legacy ``from backend.agent.loop import SYSTEM_PROMPT_DM`` still works.
from .loop_prompts import (  # noqa: F401
...
from .cron import CronRunner, build_cron_system_prompt, compose_cron_user_message
from .turn_preparer import TurnPreparer
from . import loop_confirm as _lc  # noqa: E402


# list-collection task heuristic. Triggers a higher tool-call
# budget when the user message looks like a multi-source list ask:
# "找 N 篇论文", "对比 / 综述 / SOTA / 顶刊 / 顶会", "compare / survey".
# Conservative on purpose; false positives just spend a few extra
# rounds, false negatives leave the user stuck at max-iter as before.
_LIST_TASK_PATTERNS = (
    # added Chinese numerals (一二两三四五六七八九十) to the
    # quantifier alternation. The 2026-05-11 user query "五篇顶刊论文"
    # was caught only by the "顶刊" branch below; if the user wrote
    # "找几篇 / 给我五个" without the venue keyword, the budget bump
    # would not have fired. Now both wordings hit branch #1.
    re.compile(
        r"(?:找|给我|列|推荐).{0,6}"
...
def _adaptive_tool_budget(
    user_text: str, *, base: int, list_task: int,
) -> int:
    """Decide the per-turn tool-call budget for ``user_text``.

    Returns ``list_task`` (≥ ``base``) when the message matches one of
    the list-collection patterns; otherwise the standard ``base``.
    The list-task bump is purely additive — it never *lowers* the
    budget, so a misclassified message is at worst a no-op.
    """
    text = (user_text or "").strip()
    if not text or list_task <= base:
        return base
    for pat in _LIST_TASK_PATTERNS:
        if pat.search(text):
            return list_task
    return base
...
class LoopOutcome:
    """Result of one drive of the LLM tool-use loop.

    Exactly one of ``final_text`` / ``question_message`` is populated when
    the loop terminates normally; both can be ``None`` only on a hard
    failure (LLM unavailable in cron path).

    ``tool_call_count`` and ``invoked_tool_names`` feed the review-turn
    trigger heuristic: a substantive turn (>=2 tool calls, or any
    skill-backed cron) earns a review pass.
    """
    ...
    # Per-tool-call outcomes used by FailureLearner. Tuple of
    # ``(tool_name, ok, error)`` in invocation order. Empty for paths
    # that suspended on a confirmation (the turn isn't really done so
    # we don't want to fingerprint partial state).
    tool_outcomes: tuple[tuple[str, bool, Optional[str]], ...] = ()
    # structured failure marker. ``None`` means the turn
    # finished cleanly (final_text or question_message is the real
    # answer). Non-None values let downstream consumers (delegation,
    # review) react without string-matching on user-facing text.
    # Known kinds:
    #   * "llm_error"      — provider raised mid-loop or final
    #   * "empty_response" — provider returned 0-content non-tool reply
    #   * "max_iterations" — hit budget, final answer is best-effort
    #   * "no_llm"         — verbatim echo path (no LLM configured)
    ...
    def to_message(
        ...
        *,
        ...
        """Render this outcome as an OutgoingMessage, or None on hard failure."""
...
class AgentLoop:
    ...
    def __init__(
        ...
        *,
        ...
        # adaptive cap for list-collection asks. When the
        ...
        # :mod:`backend.agent.trajectory`. Set ``trajectory_compress=False``
        ...
        # Tool health tracker for cross-turn reliability monitoring
        from .tool_reliability import ToolHealthTracker
        self._health_tracker = ToolHealthTracker()
        # Rate limiter (None = disabled)
        from ..core.security import RateLimiter
        self._rate_limiter = RateLimiter()
        # User manager for multi-user support (None = single-user mode)
    ...
    async def run_turn(
        ...
        *,
        ...
        # log attachment counts so image-only inbound messages
        ...
        # Multi-user: register user and check enabled
        if self._user_manager is not None and message.user_id:
            user = self._user_manager.get_or_create(
                message.platform, message.user_id,
            )
            if not user.get("enabled", True):
                return OutgoingMessage(
                    target=message.reply_target,
                    text="您的账号已被禁用，请联系管理员。",
                )
        ctx = TurnContext(
            turn_id=turn_id,
        ...
        with set_turn_context(ctx):
            # image-only messages must reach TurnPreparer so the
            ...
            # Session commands (/new, /reset, /summary, /export)
            # intercepted before LLM — instant response, zero token cost.
            from .session_commands import try_handle_command
            from ..core.config import get_settings
            cmd_reply = try_handle_command(
                text, session_id, self._memory,
                workspace_dir=get_settings().workspace_dir,
            )
            if cmd_reply is not None:
                return OutgoingMessage(target=message.reply_target, text=cmd_reply)

            # Security: input validation
            from ..core.security import validate_input
            validation = validate_input(text)
            if not validation.ok:
                return OutgoingMessage(target=message.reply_target, text=validation.error)

            # Security: rate limiting
            user_key = f"{message.platform}:{message.user_id or 'anon'}"
            if self._rate_limiter is not None:
                allowed, reason = self._rate_limiter.check(user_key)
                if not allowed:
                    return OutgoingMessage(target=message.reply_target, text=reason)
                self._rate_limiter.record_start(user_key)

            try:
                # Process file attachments (PDF, TXT, etc.)
                from .file_processor import process_attachments, build_agent_prompt
                from ..core.config import get_settings
                file_result = await process_attachments(
                    message.attachments,
                    workspace_dir=get_settings().workspace_dir,
                )

                # If files were processed, build agent prompt with extracted content
                if file_result.has_files and file_result.success_count > 0:
                    file_prompt = build_agent_prompt(file_result)
                    if text:
                        text = f"{text}\n\n{file_prompt}"
                    else:
                        text = file_prompt
                    ...
                    if self._memory is not None:
                        from .file_processor import build_followup_context
                        followup_ctx = build_followup_context(file_result.files)
                        if followup_ctx:
                            try:
                                self._memory.remember_file_context(
                                    session_id, followup_ctx,
                                )
                            except Exception:
                                pass  # Best-effort

                # If still no text (user sent only files without caption)
                if not text and file_result.has_files:
                    text = "请分析并总结用户发送的文件内容。"
                ...
                if self._rate_limiter is not None:
                    self._rate_limiter.record_end(user_key)
    ...
    # confirmation formatters extracted to ``loop_confirm``.
