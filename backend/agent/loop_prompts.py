"""System prompts and review-turn constants used by :mod:`backend.agent.loop`.
...
LZAGENT_CORE_PRINCIPLES = (
    "\n\n"
    "## LZAgent 核心原则（精简版，详见 docs/architecture_principles.md）\n"
...
SYSTEM_PROMPT_DM = (
    "你是 LZAgent，一名常驻 IM 助手。请简洁、同语种回复；需要外部信息时可调用工具，不要编造 URL 或文件内容。"
...
SYSTEM_PROMPT_CRON = (
    "你是 LZAgent，一名常驻 IM 助手，正在执行用户提前设定的定时任务。"
...
SKILL_REVIEW_PROMPT = (
    "你是 LZAgent，正在做一次「**日内会后复盘**」(intra-day review)。"
    ...
    "人物关系（老婆叫小明）、长期目标（在做 LZAgent 项目）、习惯。"
...
END_OF_DAY_REVIEW_PROMPT = (
    "你是 LZAgent，正在做一次「**日终 / 阶段完成复盘**」(end-of-day"
...
# merges near-duplicates deterministically and only archives —
...
# IM users have an "外围随意安装 MCP" path: any platform may call
...
def _current_time_prompt_block() -> str:
    ...
    granularity dropped from minute to *day* on purpose.
