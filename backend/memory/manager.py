...
Mirrors Hermes' ``agent/memory_manager.py`` for the LZAgent single-user
...
def sanitize_untrusted(text: str) -> str:
    ...
    Run on every string that originated outside LZAgent's own system
...
class MemoryManager:
    ...
    def _format_entries(entries: list[dict[str, Any]]) -> str:
        ...
        defence-in-depth: also scrub each rendered entry
    ...
    def prefetch(self, query: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # do NOT touch_recall during prefetch. The
    ...
    def sync_turn(
        ...
        *,
        ...
        if not user_content and not assistant_content:
            return
        # scrub hallucinated tool-call markup from the
    ...
    def remember_file_context(self, session_id: str, context: str) -> None:
        """Store file extraction context in session for follow-up questions.

        This is a lightweight in-memory store keyed by session_id.
        The context is available for the next few turns so the agent
        can answer follow-up questions about previously extracted files.
        """
        if not hasattr(self, '_file_contexts'):
            self._file_contexts: dict[str, str] = {}
        self._file_contexts[session_id] = context
        logger.debug("[memory] stored file context for session {}", session_id)

    def get_file_context(self, session_id: str) -> str:
        """Retrieve stored file context for a session."""
        if not hasattr(self, '_file_contexts'):
            return ""
        return self._file_contexts.get(session_id, "")

    def clear_file_context(self, session_id: str) -> None:
        """Clear file context for a session."""
        if hasattr(self, '_file_contexts'):
            self._file_contexts.pop(session_id, None)
    ...
    def on_pre_compress(self, history: list[Any]) -> str:
        ...
        Called by :class:`backend.agent.summary_compressor.SummaryCompressor`
