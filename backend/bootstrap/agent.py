...
def build_failure_learner(settings: "Settings", memory_store: Any) -> Any:
    if not settings.failure_learning_enabled:
        return None
    from ..agent.failure_learning import FailureLearner
    # persist counters so a docker restart doesn't reset
...
def build_summary_compressor(
    ...
    if not settings.context_summary_enabled:
        return None
    from ..agent.summary_compressor import SummaryCompressor
