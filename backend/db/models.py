...
class PendingConfirmation(Base):
    """A paused tool invocation waiting for user approval.
    ...
    :func:`backend.agent.decisions.classify_decision` — resolves this row
...
class User(Base):
    """User record for multi-user support.

    Users are identified by ``platform:user_id`` (e.g., ``weixin:o9cq80-xxx``).
    Each user gets their own ``knowledge_base_id`` for memory isolation.
    """

    __tablename__ = "users"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    user_key: Mapped[str] = mapped_column(String(512), unique=True, index=True)
    platform: Mapped[str] = mapped_column(String(32))
    user_id: Mapped[str] = mapped_column(String(256))
    display_name: Mapped[str] = mapped_column(String(256), default="")
    role: Mapped[str] = mapped_column(String(16), default="user")  # admin|user|guest
...
class MCPServerRegistration(Base):
    ...
    * ``rest``     — reserved for the future ``POST /api/mcp/servers``
    ...
    description: Mapped[str] = mapped_column(Text, default="")
    enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    # per-tool permission overrides (e.g. {"geocode": "safe"}).
    # JSON-encoded dict[str, "safe" | "confirm"]. Default '{}' covers
    # rows created before they keep the existing
...
def apply_lightweight_migrations(engine) -> None:
    ...
    ADDITIVE_COLUMNS: list[tuple[str, str, str]] = [
        ...
        # per-tool permission overrides for attached MCP
